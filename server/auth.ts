...
import type { InsertUser, User as SelectUser } from "../shared/schema.js";
...
import { aadhaarVerifications, insertAadhaarVerificationSchema } from "@shared/schema.js";
...
export function setupAuth(app: Express) {
    ...
    try {
      ...
      if (!isVerifyingOtp) {
        ...
        if ('confirmPassword' in dataWithoutConfirmPassword) {
          delete (dataWithoutConfirmPassword as any).confirmPassword;
        }
  ...
  const profilePasswordHandler = async (req: Request, res: Response) => {
    if (!req.isAuthenticated()) {
  ...
  app.post("/api/register", registerHandler as RequestHandler);
  ...
  app.get("/api/user", userHandler as RequestHandler);
  app.patch("/api/profile-password", profilePasswordHandler as RequestHandler);
}